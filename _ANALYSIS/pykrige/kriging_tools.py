...
def write_asc_grid(x, y, z, filename='output.asc', style=1):
    ...
    with io.open(filename, 'w') as f:
        if style == 1:
            ...
            f.write("XLLCENTER      " + '{:<10.10f}'.format(xllcenter) + '\n')
            f.write("YLLCENTER      " + '{:<10.10f}'.format(yllcenter) + '\n')
            f.write("DX             " + '{:<10.10f}'.format(dx) + '\n')
            f.write("DY             " + '{:<10.10f}'.format(dy) + '\n')
            f.write("NODATA_VALUE   " + '{:<10.2f}'.format(no_data) + '\n')
        elif style == 2:
            f.write("NCOLS          " + '{:<10n}'.format(ncols) + '\n')
            f.write("NROWS          " + '{:<10n}'.format(nrows) + '\n')
            f.write("XLLCORNER      " + '{:<10.10f}'.format(xllcorner) + '\n')
            f.write("YLLCORNER      " + '{:<10.10f}'.format(yllcorner) + '\n')
            f.write("CELLSIZE       " + '{:<10.10f}'.format(cellsize) + '\n')
            f.write("NODATA_VALUE   " + '{:<10.2f}'.format(no_data) + '\n')
        else:
